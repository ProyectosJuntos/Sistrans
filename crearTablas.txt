...
CREATE TABLE B_PUESTOATENCION
   ...
   TIPOPUESTO VARCHAR2(255 BYTE) NOT NULL,
    ...
    TIPOIDCAJERO VARCHAR2(255 BYTE),
   IDCAJERO NUMBER,
   IDOFICINA NUMBER,
...
CREATE TABLE B_OPERACION
   ...
   IDPUESTOATENCION NUMBER NOT NULL,
   TIPOIDUSUARIO VARCHAR2(255 BYTE) NOT NULL,
   IDUSUARIO NUMBER NOT NULL,
...
(IDCUENTAJURIDICA NUMBER NOT NULL,
...
ALTER TABLE B_USUARIO
	ADD CONSTRAINT CK_TIPOID_PPTO
	CHECK (TIPOID IN ('CI','CC','TP','CE'))
ENABLE;
...
CREATE index Date_fechaRealizada on B_operacion (fecha);
CREATE index Date_fechaCreacion on B_producto (fechaCreacion);
CREATE index Date_fechaUltimoMov on B_producto (fechaUltimoMov);
CREATE BITMAP INDEX FK_idUsuario ON b_producto (tipoIdUsuario, idUsuario);
